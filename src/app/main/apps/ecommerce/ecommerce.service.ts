...
export class EcommerceService implements Resolve<any> {
 ...
 url = 'http://localhost:9090/offreService';
  newUrl : string
  OFFRESERVICES = [];
  url2 = 'http://localhost:9090/commande';
  newUrl2: string;
  COMMANDES = [];
  ...
  constructor(private _httpClient: HttpClient) {
    // Initialize cartList to an empty array
  this.cartList = [];
  this.wishlist = [];

  // Initialize the other properties and behaviorsubjects
  ...
  getProducts(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this._httpClient.get<OffreService[]>(this.url+"/",this.httpOptions).subscribe((response: any) => {
  ...
  getSelectedProduct(): Promise<any[]> {
    return this.getProducts().then(() => this.getSelectedProduct());
  ...
  addCommande (commande: Commande): Observable<Commande> {
    return this._httpClient.post<Commande>(this.url2+"/add", commande,
    this.httpOptions);
