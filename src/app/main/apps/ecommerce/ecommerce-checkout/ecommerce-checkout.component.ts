...
export class EcommerceCheckoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.products.forEach(product => {
      product.isInWishlist = Array.isArray(this.wishlist) && this.wishlist.findIndex(p => p.productId === product.id) > -1;
  ...
  save() : void {
    this._ecommerceService.addCommande(this.commande).subscribe ((res) => {
